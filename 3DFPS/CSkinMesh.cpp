...
void CSkinMesh::Render(
	...
	const FOG& Fog, LPD3DXANIMATIONCONTROLLER pAC)
{
	m_mView = mView;
	m_mProj = mProj;
	m_Light = Light;
...
void CSkinMesh::SendCBufferPerFrame()
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		//----- ライト情報 -----.
		cb.LightColor = D3DXVECTOR4(1.0f, 1.0f, 1.0f, 1.0f); //白色のライト.
		cb.AmbientColor = D3DXVECTOR4(0.4f, 0.4f, 0.4f, 1.0f); //弱い環境光.
		//ライト方向.
		...
		cb.FogParams = D3DXVECTOR4(m_Fog.Start, m_Fog.End, m_Fog.Density * m_Fog.Enable, (float)m_Fog.Mode); //開始, 終了, 密度, モード.
